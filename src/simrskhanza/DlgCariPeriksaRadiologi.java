   ...
   if(tabMode.getRowCount()!=0){
    ...
    private void BtnPrint1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint1ActionPerformed
        if(Kd2.getText().equals("")){
            ...
            Sequel.cariIsi("select png_jawab from penjab inner join reg_periksa on penjab.kd_pj=reg_periksa.kd_pj where no_rkm_medis=?", Penjab, NoRM.getText());
...
//          finger=Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",kdpenjab);
            ...
            pilihan = (String)JOptionPane.showInputDialog(null,"Silahkan pilih hasil pemeriksaan..!","Hasil Pemeriksaan",JOptionPane.QUESTION_MESSAGE,null,new Object[]{"Hasil Pemeriksaan Radiologi","Hasil Pemeriksaan USG","PDF Hasil Pemeriksaan Radiologi","PDF Hasil Pemeriksaan USG"},"Hasil Pemeriksaan Radiologi");
            switch (pilihan) {
                case "Hasil Pemeriksaan Radiologi":
                      Valid.MyReport("rptPeriksaRadiologi.jasper","report","::[ Hasil Pemeriksaan Radiologi ]::",param);
                      break;
                case "Hasil Pemeriksaan USG":
                      Valid.MyReport("rptPeriksaRadiologi2.jasper","report","::[ Hasil Pemeriksaan USG ]::",param);
                      break;
                case "PDF Hasil Pemeriksaan Radiologi":
                      Valid.MyReportPDF("rptPeriksaRadiologi.jasper","report","::[ PDF Hasil Pemeriksaan Radiologi ]::",param);
                      break;
                case "PDF Hasil Pemeriksaan USG":
